%% 1D spring-slider model with slip-weakening friction
% 03/22/2022
...
% mu is determined by the failure law. In this case, slip-weakening friction (SW):

% mu =
%      mu_s - (mu_s-mu_d)*D/D_c            if D <= D_c
%      mu_d                                if D > D_c

% where mu_s and mu_d are the static and dynamic friction coefficients,
% and D_c is the slip-weakening distance.
...
D_c     = 0.9796528896;          % characteristic slip distance

%% Initial and reference values
V_0     = 1e-3;                  % velocity of the load point
V_in    = 1*V_0;                 % initial sliding velocity
...
tf      = 2147.5*pi*sqrt(M/K_s);            % simulation length 
...
%% Numerically solve 1D spring-slider equations with SW failure law
global sl_ref
sl_ref = 0;
global k
k = 0;
...
%% Non-dimentionalize
xi      = K_s*D_c/Sn;
gamma   = sqrt(K_s/M)*D_c/V_0;
Tf      = tf/(D_c/V_0);

%% dy = [du/dT dv/dT]'

dy      = @(t,y) [                        y(2)-1; 
                  -gamma*gamma*(y(1) + (1/xi)*tau_sw(t, y(1), y(2), K_s, Sn, mu_s, mu_d, D_c, V_0))];

% [u_0 v_0] initial guess
y_in    = [u_0/D_c V_0/V_0];  

% Jacobian
jac     = @(t,y) [0 1;
                 -gamma*gamma*(1+(1/xi)*jac_y1_sw(t, y(1), y(2), K_s, Sn, mu_s, mu_d, D_c, V_0)) 0];
...
for i = 1:length(Y(:,1))
    tau(i) = Sn*tau_sw(T(i), Y(i,1), Y(i,2), K_s, Sn, mu_s, mu_d, D_c, V_0);
end
ac      = -1/M*(K_s*u + tau);
...
%% Function for tau SW
function ta = tau_sw(t, u1, u2, K_s, Sn, mu_s, mu_0, D_c, V_star) 
global sl_ref
global k
global V_ei
...
vel_tol = V_ei/V_star;
    if u2 > vel_tol 
        ...
        if u1 + t - sl_ref  <= 1
            ta = -sign(u1)*(mu_0 + (mu_s-mu_0).*(1-(u1 - sl_ref + t)));
        else
            ta = -sign(u1)*mu_0;
        end
...
%% Function for Jacobian SW
function jc = jac_y1_sw(t, u1, u2, K_s, Sn, mu_s, mu_0, D_c, V_star) 
global sl_ref_j
global k_j
global V_ei
...
vel_tol = V_ei/V_star;
    if u2 > vel_tol 
        ...
        if u1 + t - sl_ref_j  <= 1
            ta = -sign(u1)*(mu_0 + (mu_s-mu_0).*(1-(u1 - sl_ref_j + t)));
            jc = sign(u1)*(mu_s-mu_0);
        else
            ta = -sign(u1)*mu_0;
            jc = 0;
        end
