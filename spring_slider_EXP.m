%% 1D spring-slider model with exponential friction
% 03/22/2022
...
% mu is determined by the failure law. In this case, exponential
% failure law (ECZ):

% mu = mu_d + (mu_s-mu_d)*(D+D_1)/D_2* e^(1-(D+D_1)/D_2),

% where mu_s and mu_d are the static and dynamic friction coefficients,
% and D_1 and D_2 are the two slip distances which we call 
% "slip-shift" and "slip-stretch" respectively.
...
s_sh    = 0.0845311;
s_str   = 0.244443;              % length parameters for exp coh zone
mu_i    = mu_d + (mu_s-mu_d).*s_sh/s_str.*exp(1-(s_sh/s_str));
...
%% Numerically solve 1D spring-slider equations with EXP failure law
global sl_ref_e
sl_ref_e = 0;
global k_e
k_e = 0;
...
%% dy = [du/dT dv/dT]'
dy      = @(t,y) [                        y(2)-1; 
                  -gamma*gamma*(y(1) + (1/xi)*tau_exp(t, y(1), y(2), K_s, Sn, mu_s, mu_d, s_sh, s_str, V_0))];
...
% Jacobian
jac     = @(t,y) [0 1;
                 -gamma*gamma*(1+(1/xi)*jac_y1_exp(t, y(1), y(2), K_s, Sn, mu_s, mu_d, s_sh, s_str, V_0)) 0];
...
for i = 1:length(Y(:,1))
    tau(i) = Sn*tau_exp(T(i), Y(i,1), Y(i,2), K_s, Sn, mu_s, mu_d, s_sh, s_str, V_0);
end
ac      = -1/M*(K_s*u + tau);
...
%% Save to file
if sd == 1
    EXP = [t(lmin(pn):lmin(pn+1))-t(lmin(pn)) v(lmin(pn):lmin(pn+1)) u_abs(lmin(pn):lmin(pn+1))-u_abs(lmin(pn)) tau(lmin(pn):lmin(pn+1)) ac(lmin(pn):lmin(pn+1))*M+tau(lmin(pn):lmin(pn+1))];
    save('Data_FL/EXP.mat', 'EXP');
end

%% Function for tau exponential
function ta = tau_exp(t, u1, u2, K_s, Sn, mu_s, mu_d, slip_shift, slip_stretch, V_star) 
global sl_ref_e
global k_e
global V_ei
...
vel_tol = V_ei/V_star;
    if u2 > vel_tol
        ...
        ta = -sign(u1)*(mu_d + (mu_s-mu_d).*(u1 - sl_ref_e + t + slip_shift/slip_stretch).*exp(1-(u1 - sl_ref_e + t) - slip_shift/slip_stretch));
    else
        ta = -sign(u1)*(mu_d + (mu_s-mu_d).*slip_shift/slip_stretch.*exp(1-(slip_shift/slip_stretch)));
...
function jc = jac_y1_exp(t, u1, u2, K_s, Sn, mu_s, mu_d, slip_shift, slip_stretch, V_star) 
global sl_ref_e_j
global k_e_j
global V_ei
...
vel_tol = V_ei/V_star;
    if u2 > vel_tol
        ...
        ta = -sign(u1)*(mu_d + (mu_s-mu_d).*(u1 - sl_ref_e_j + t + slip_shift/slip_stretch).*exp(1-(u1 - sl_ref_e_j + t) - slip_shift/slip_stretch));
        jc = sign(u1)*(mu_s-mu_d)*(u1 - sl_ref_e_j + t - 1 + slip_shift/slip_stretch)*exp(1-(u1 - sl_ref_e_j + t) - slip_shift/slip_stretch);    
